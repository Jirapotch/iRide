...
export interface MediaRepository {
  ...
  readonly findOwnedUpload: (
    userId: string,
    id: string,
  ) => Promise<OwnedUpload | null>;
  readonly markProcessingAndEnqueue: (
    userId: string,
    id: string,
    message: ProcessingMessage,
  ) => Promise<void>;
  readonly findDeliverableVariant: (
    id: string,
...
export function handleMediaUpload(
  ...
  return execute(request, dependencies, async () => {
    ...
    const input = await read<MediaUploadRequest>(
      request,
...
export function handleMediaComplete(
  request: Request,
  id: string,
  dependencies = productionDependencies(),
) {
  return execute(request, dependencies, async () => {
    ...
    const storageProvider = media.storageProvider ?? "r2";
    const object = await dependencies.storage.head(
      media.objectKey,
      storageProvider,
    );
    if (object.bytes !== media.bytes || object.contentType !== media.mimeType)
      throw new MediaError("MEDIA_UPLOAD_INVALID", 400);
    const message: ProcessingMessage = {
      version: 1,
      jobId: randomUUID(),
      idempotencyKey: `media:${id}:process`,
      attempt: 0,
      mediaId: id,
      ownerId: auth.userId,
    ...
    await dependencies.repository.markProcessingAndEnqueue(
      auth.userId,
      id,
      message,
    );
...
export function handleMediaVariant(
  request: Request,
  id: string,
  kind: string,
  dependencies = productionDependencies(),
  ...
  return execute(request, dependencies, async () => {
    ...
    const viewer = await optionalViewer(request, dependencies);
    const variant = await dependencies.repository.findDeliverableVariant(
      id,
      kind,
      viewer,
    );
    ...
    return new Response(null, {
      status: 307,
      headers: {
        Location: await dependencies.storage.signDownload(
          variant.objectKey,
          120,
...
export function handleMediaOptions(
  request: Request,
  allowed = process.env.CORS_ALLOWED_ORIGINS,
) {
  const cors = createCorsDecision(request, allowed, "GET, POST, OPTIONS");
  return new Response(null, {
...
async function execute(
  ...
  if (!cors.allowed) return error("CONTENT_FORBIDDEN", 403, cors.headers);
  try {
    ...
    if (reason instanceof AuthenticationError)
      return Response.json(toAuthErrorBody(reason), {
        status: reason.status,
        headers: cors.headers,
      });
    const item = reason as { code?: string; status?: number };
    return error(
      item.code ?? "MEDIA_UNAVAILABLE",
      item.status ?? 503,
      cors.headers,
    );
  }
...
async function read<T>(
  ...
  try {
    ...
    throw new MediaError("MEDIA_VALIDATION_FAILED", 400);
  }
  const result = schema.safeParse(body);
  if (!result.success) throw new MediaError("MEDIA_VALIDATION_FAILED", 400);
  return result.data as T;
}
async function optionalViewer(request: Request, deps: MediaDependencies) {
...
async function requireActiveAccount(
  ...
  const access = await repository.getAccountAccess(userId);
  if (access?.status !== "active" || access.transitionId !== null)
...
function error(code: string, status: number, headers?: Headers) {
  return Response.json(
...
function productionDependencies(): MediaDependencies {
  ...
  if (
    ...
    return {
      ...
      newId: randomUUID,
      repository: new Proxy({}, { get: () => unavailable }) as MediaRepository,
      storage: {
  ...
  return {
    ...
    newId: randomUUID,
    repository: createMediaRepository({ url, publishableKey, serviceRoleKey }),
