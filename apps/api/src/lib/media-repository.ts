...
export function createMediaRepository(config:Config):MediaRepository{
  ...
  return{
    async getAccountAccess(userId){const {data,error}=await admin.from("account_access").select("status,transition_id").eq("user_id",userId).maybeSingle();ensure(error);return data?{status:data.status,transitionId:data.transition_id}:null},
...
async function isHiddenOwner(admin:ReturnType<typeof createAdminDatabaseClient>,ownerId:string){
...
async function hasVisibleOwner(admin:ReturnType<typeof createAdminDatabaseClient>,ownerIds:readonly string[]){
  const unique=[...new Set(ownerIds)];if(!unique.length)return false;
  const {data,error}=await admin.from("account_access").select("user_id,status,transition_id").in("user_id",unique);ensure(error);
  return (data??[]).some((row)=>row.status!=="suspended"&&row.transition_id===null);
}
