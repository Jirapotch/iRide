import { createEventSchema } from "@iride/validation";
...
import type { ContentRepository, SearchType } from "./content";
...
export function createContentRepository(
  ...
  return {
    ...
    async getPost(id, viewerId) {
      ...
      if (
        !row ||
        (row.deleted_at && row.author_id !== viewer.userId && !viewer.canManage)
      )
        return null;
    ...
    async createPost(userId, accessToken, input) {
      ...
      const { data: id, error } = await ownerClient(accessToken).rpc(
        "save_post_with_markers",
        {
          target_post_id: null as unknown as string,
      ...
      const data = await findPost(admin, id!);
      ensureWrite(null, data);
      return (
        await postDtos(admin, [data!], await viewerCapabilities(admin, userId))
      )[0]!;
    },
    async updatePost(userId, accessToken, id, input) {
      ...
      await validateMarkerTags(admin, input.markerTags ?? []);
      const { data: savedId, error } = await ownerClient(accessToken).rpc(
        "save_post_with_markers",
        {
          target_post_id: id,
          post_body: input.body,
    ...
    async getEvent(id, viewerId) {
      ...
      if (
        !row ||
        (row.deleted_at &&
          row.organizer_id !== viewer.userId &&
          !viewer.canManage)
      )
        return null;
    ...
    async createEvent(userId, accessToken, input) {
      ...
      ensureWrite(error, data);
      return (
        await eventDtos(admin, [data!], await viewerCapabilities(admin, userId))
      )[0]!;
    },
    async updateEvent(userId, accessToken, id, input) {
      ...
      const parsed = createEventSchema.safeParse(mergeEvent(current!, input));
      if (!parsed.success)
        throw repositoryError("CONTENT_VALIDATION_FAILED", 400);
      const merged = parsed.data as CreateEventInput;
      const { data, error } = await ownerClient(accessToken)
        .from("events")
    ...
    async search(query, types, viewerId) {
      return searchContent(
        ...
        query,
        types,
        await viewerCapabilities(admin, viewerId),
      );
    },
  };
}
...
async function authors(
  ...
  const statusByUserId = new Map(
    (access ?? []).map((row) => [row.user_id, row]),
  );
  return new Map(
    (data ?? [])
      .filter((row) => {
...
async function postDtos(
  ...
  if (postIds.length) {
    ...
    const statuses = new Map(
      (access ?? []).map((item) => [item.user_id, item]),
    );
    for (const item of data ?? []) {
      const access = statuses.get(item.author_id);
      if (
        access?.transition_id === null &&
        (viewer.canManage || access.status !== "suspended")
      ) {
...
async function postMarkerTags(
  ...
  const { data, error } = await admin
    .from("post_marker_tags")
    .select("*")
    .in("post_id", [...postIds])
    .order("position");
  ensureQuery(error);
  const eventIds = (data ?? []).flatMap((row) =>
    row.event_id ? [row.event_id] : [],
  );
  const { data: events, error: eventsError } = eventIds.length
    ? await admin
        .from("events")
        .select("id,title,kind,deleted_at,organizer_id")
        .in("id", eventIds)
    : { data: [], error: null };
  ...
  const people = await authors(
    admin,
    (events ?? []).map((row) => row.organizer_id),
    viewer.canManage,
  );
  for (const row of data ?? []) {
    const target = row.event_id ? eventMap.get(row.event_id) : undefined;
    const targetVisible = Boolean(
      target && !target.deleted_at && people.has(target.organizer_id),
    );
    const value = {
      kind: "event" as const,
      id: row.event_id!,
      title: targetVisible ? target!.title : null,
      markerKind: targetVisible
        ? normalizeExploreKind(String(target!.kind))
        : null,
      available: targetVisible,
    };
...
async function validateMarkerTags(
  admin: AdminClient,
  tags: NonNullable<CreatePostInput["markerTags"]>,
) {
  const eventIds = tags
  ...
  if ((events.data ?? []).length !== eventIds.length)
    throw repositoryError("CONTENT_VALIDATION_FAILED", 400);
}

async function eventDtos(
  admin: AdminClient,
  rows: Tables<"events">[],
  viewer: ViewerCapabilities,
): Promise<EventDto[]> {
      ...
      ? [
          {
            ...
            canEdit:
              viewer.canManage ||
              (viewer.canWrite && row.organizer_id === viewer.userId),
            createdAt: row.created_at,
            updatedAt: row.updated_at,
          },
        ]
  ...
  return {
    ...
    locationLabel:
      input.locationLabel === undefined
        ? row.location_label
        : input.locationLabel,
    latitude: input.latitude === undefined ? row.latitude : input.latitude,
    longitude: input.longitude === undefined ? row.longitude : input.longitude,
    destinationLabel:
    ...
    startsAt: input.startsAt === undefined ? row.starts_at : input.startsAt,
    stops:
      input.stops ??
...
function ensureWrite(
  ...
  if (error) {
    ...
    throw repositoryError(
      forbidden ? "CONTENT_FORBIDDEN" : "CONTENT_UPDATE_FAILED",
      forbidden ? 403 : 503,
      error,
    );
  }
...
function normalizeExploreKind(value: string): "meeting" | "event" | "trip" {
  if (value === "meeting" || value === "event" || value === "trip")
    return value;
...
async function searchContent(
  ...
  if (types.includes("profiles")) {
    ...
    ensureQuery(byUsername.error);
    ensureQuery(byName.error);
    const profiles = new Map(
      [...(byUsername.data ?? []), ...(byName.data ?? [])].map((row) => [
        row.id,
        row,
      ]),
    );
    ...
    for (const row of profiles.values())
      if (row.username && row.display_name && people.has(row.id))
        results.push({
          id: row.id,
          kind: "profile",
  ...
  if (types.includes("events")) {
    const { data, error } = await admin
      .from("events")
      .select("id,title,location_label,destination_label,organizer_id")
      .is("deleted_at", null)
      .ilike("title", pattern)
      .limit(8);
    ensureQuery(error);
    const people = await authors(
      admin,
      (data ?? []).map((row) => row.organizer_id),
      viewer.canManage,
    );
    for (const row of data ?? [])
      if (people.has(row.organizer_id))
        results.push({
          id: row.id,
          kind: "event",
          title: row.title,
