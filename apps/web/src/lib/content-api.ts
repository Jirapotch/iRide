...
export function createComment(
  ...
  return contentMutation<CommentDto>(
    `/api/v1/posts/${encodeURIComponent(postId)}/comments`,
    accessToken,
    "POST",
    input,
...
export function deleteComment(accessToken: string, id: string) {
  return contentDelete(
    `/api/v1/comments/${encodeURIComponent(id)}`,
    accessToken,
  );
}
export function getGarage(username: string, accessToken?: string) {
  return contentGet<VehicleDto[]>(
    `/api/v1/users/${encodeURIComponent(username)}/garage`,
    accessToken,
  );
}
export function getProfileActivities(username: string, accessToken?: string) {
  return contentGet<ExploreFeatureDto[]>(
    `/api/v1/users/${encodeURIComponent(username)}/activities`,
    accessToken,
  );
}
export function getVehicle(id: string, accessToken?: string) {
  return contentGet<VehicleDto>(
...
export function createVehicle(accessToken: string, input: CreateVehicleInput) {
  return contentMutation<VehicleDto>(
    "/api/v1/vehicles",
    accessToken,
    "POST",
    input,
...
export function deleteVehicle(accessToken: string, id: string) {
  return contentDelete(
    `/api/v1/vehicles/${encodeURIComponent(id)}`,
    accessToken,
  );
...
export function completeMediaUpload(accessToken: string, id: string) {
  return contentMutation<{ mediaId: string; status: string }>(
    `/api/v1/media/${encodeURIComponent(id)}/complete`,
    accessToken,
    "POST",
    {},
  );
...
export function mediaVariantUrl(
  ...
  return `/api/media/${encodeURIComponent(id)}/${kind}`;
}

export function getPost(id: string, accessToken?: string) {
  return contentGet<PostDto>(
...
export function updatePost(
  ...
  return contentMutation<PostDto>(
    `/api/v1/posts/${encodeURIComponent(id)}`,
    accessToken,
    "PATCH",
    input,
...
export function createEvent(accessToken: string, input: CreateEventInput) {
  return contentMutation<EventDto>(
    "/api/v1/events",
    accessToken,
    "POST",
    input,
...
export async function getExploreContent(
  ...
  if (typeof window !== "undefined") {
    ...
    if (!response.ok)
      throw new ContentApiError("EXPLORE_UNAVAILABLE", response.status);
...
async function contentDelete(pathname: string, accessToken: string) {
  const response = await fetch(apiUrl(pathname), {
    method: "DELETE",
    cache: "no-store",
    headers: { Authorization: `Bearer ${accessToken}` },
