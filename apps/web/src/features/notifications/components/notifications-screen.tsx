...
import { useMockApp } from "@/features/notifications/mock-app-provider";

const shell = "mx-auto w-full max-w-[1180px] px-4 py-5 sm:px-6 sm:py-8";

export function NotificationsScreen({ locale }: { readonly locale: Locale }) {
        ...
        {notifications.map((item) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                dispatch({ type: "read-notification", notificationId: item.id })
              }
              key={item.id}
              type="button"
            >
