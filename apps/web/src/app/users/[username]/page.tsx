...
export default async function UserProfilePage({
  ...
  const [profile, ownProfile, vehicles, activities] = await Promise.all([
    ...
    getProfileActivities(username, session?.accessToken).catch(() => []),
  ]);
  if (!profile) notFound();
  const ownerProfile =
  ...
  return (
    ...
    >
      <UserProfileScreen
        ...
        {...(query.vehicle ? { selectedVehicleId: query.vehicle } : {})}
      />
    </AppShell>
  );
}
