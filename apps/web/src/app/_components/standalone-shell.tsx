...
export function StandaloneShell({ children, headerAction, locale, wide = false }: Readonly<{ children: ReactNode; headerAction?: ReactNode; locale: Locale; wide?: boolean }>) {
  return <div className="min-h-screen bg-background text-foreground" data-ui="standalone-shell">
    ...
    <main className="grid min-h-screen pt-[var(--header-height)] lg:grid-cols-[minmax(0,1.05fr)_minmax(28rem,.95fr)]">
      <section className="standalone-brand-art relative hidden min-h-[calc(100vh-var(--header-height))] overflow-hidden lg:block"><div aria-hidden="true" className="standalone-route-line"/><div className="absolute inset-x-10 bottom-10 rounded-3xl border border-border bg-background/78 p-7 backdrop-blur-md"><p className="premium-kicker">iRide</p><h2 className="mt-2 text-4xl font-black tracking-[-0.05em]">{locale === "th" ? "ถนนเส้นเดียวกัน เรื่องราวนับไม่ถ้วน" : "One road. Endless stories."}</h2></div></section>
