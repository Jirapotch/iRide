...
export function ProfileForm({ locale, profile }: { locale: Locale; profile?: Profile }) {
  ...
  return <form action={action} className="space-y-5"><div className="grid gap-5 sm:grid-cols-2"><Field label="Username" name="username" defaultValue={profile?.username} error={state.fieldErrors?.username?.[0]} /><Field label={locale === "th" ? "ชื่อที่แสดง" : "Display name"} name="displayName" defaultValue={profile?.displayName} error={state.fieldErrors?.displayName?.[0]} /></div><div className="space-y-2"><Label htmlFor="bio">Bio</Label><Textarea id="bio" name="bio" defaultValue={profile?.bio ?? ""} maxLength={180} rows={4} /></div><Field label={locale === "th" ? "สถานที่" : "Location"} name="location" defaultValue={profile?.location ?? ""} /><div className="grid gap-5 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="avatar">Avatar</Label><Input id="avatar" name="avatar" type="file" accept="image/jpeg,image/png,image/webp" /></div><div className="space-y-2"><Label htmlFor="cover">{locale === "th" ? "รูปภาพหน้าปก" : "Cover photo"}</Label><Input id="cover" name="cover" type="file" accept="image/jpeg,image/png,image/webp" /></div></div><ImageUploadHint locale={locale} />{state.message && <p role={state.ok ? "status" : "alert"} className={state.ok ? "text-sm text-emerald-600" : "rounded-xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive"}>{state.message}</p>}<Button disabled={pending}>{pending ? (locale === "th" ? "กำลังบันทึก…" : "Saving…") : (locale === "th" ? "บันทึกโปรไฟล์" : "Save profile")}</Button></form>;
}
