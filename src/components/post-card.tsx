...
export function PostCard({ post, locale }: { post: Post; locale: Locale }) {
  ...
  function toggleLike() {
    ...
    startTransition(async () => {
      ...
      const result = await toggleLikeAction(locale, post.id, optimistic.liked);
  ...
  return (
    <Card className="surface-shadow overflow-hidden border-white/70 bg-card/95 py-0">
      <CardHeader className="flex flex-row items-center gap-3 px-4 pt-4 pb-3 sm:px-5">
        <Link href={`/${locale}/profile/${post.author.username}`}><Avatar className="size-10"><AvatarImage src={post.author.avatarUrl ?? undefined} alt={post.author.displayName} /><AvatarFallback>{initials(post.author.displayName)}</AvatarFallback></Avatar></Link>
        <div className="min-w-0 flex-1">
          <Link href={`/${locale}/profile/${post.author.username}`} className="truncate text-sm font-semibold hover:text-primary">{post.author.displayName}</Link>
        ...
        <Button variant="ghost" size="icon" aria-label="Post menu"><MoreHorizontal className="size-4" /></Button>
      </CardHeader>
      ...
      <CardFooter className="flex-col items-stretch gap-2 px-3 py-2 sm:px-4">
        <div className="flex items-center justify-between">
          ...
          <Button asChild variant="ghost" size="sm" className="gap-2"><Link href={`/${locale}/post/${post.id}`}><MessageCircle className="size-4" />{post.commentsCount}</Link></Button>
          <Button variant="ghost" size="sm" className="gap-2"><Send className="size-4" /><span className="hidden sm:inline">Share</span></Button>
        </div>
