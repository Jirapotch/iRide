...
export async function getFeed(): Promise<Post[]> {
  ...
  const { data, error } = await supabase
    .from("posts")
    .select("id,body,photo_url,created_at,profiles!posts_author_id_fkey(username,display_name,avatar_url),vehicles(id,nickname,make,model,year),likes(count),comments(count)")
  ...
  return Promise.all(data.map(async (row: Record<string, unknown>) => {
    const path = row.photo_url ? String(row.photo_url) : null;
    const { data: signed } = path
      ? await supabase.storage.from("post-media").createSignedUrl(path, 60 * 60)
      : { data: null };
    return mapPost(row, signed?.signedUrl ?? null, likedPostIds.has(String(row.id)));
  }));
}
...
export async function getPost(id: string): Promise<Post | null> {
  ...
  const { data } = await supabase
    .from("comments")
    .select("id,body,created_at,profiles!comments_author_id_fkey(username,display_name,avatar_url)")
    .eq("post_id", id)
  ...
  const comments: Comment[] = (data ?? []).map((row) => {
    ...
    return { id: row.id, body: row.body, createdAt: row.created_at, author: { username: author?.username ?? "driver", displayName: author?.display_name ?? "iRide driver", avatarUrl: author?.avatar_url ?? null } };
  });
...
export async function getPublicProfile(username: string): Promise<{ profile: Profile; vehicles: Vehicle[] } | null> {
  ...
  const profile: Profile = {
    id: profileRow.id, username: profileRow.username, displayName: profileRow.display_name, bio: profileRow.bio, location: profileRow.location, avatarUrl: profileRow.avatar_url, locale: profileRow.locale, followersCount: profileStats?.followers_count ?? 0, followingCount: profileStats?.following_count ?? 0,
  };
  const vehicles: Vehicle[] = (vehicleRows ?? []).map((v) => ({
    id: v.id, ownerId: v.owner_id, nickname: v.nickname, make: v.make, model: v.model, year: v.year, trim: v.trim, color: v.color, description: v.description, coverUrl: v.cover_url,
  }));
...
export async function getMyVehicles(): Promise<Vehicle[]> {
  ...
  return (data ?? []).map((v) => ({ id: v.id, ownerId: v.owner_id, nickname: v.nickname, make: v.make, model: v.model, year: v.year, trim: v.trim, color: v.color, description: v.description, coverUrl: v.cover_url }));
}

function mapPost(row: Record<string, unknown>, photoUrl = row.photo_url ? String(row.photo_url) : null, likedByViewer = false): Post {
  const author = (Array.isArray(row.profiles) ? row.profiles[0] : row.profiles) as Record<string, string | null> | null;
  ...
  return {
    ...
    author: { username: author?.username ?? "driver", displayName: author?.display_name ?? "iRide driver", avatarUrl: author?.avatar_url ?? null },
